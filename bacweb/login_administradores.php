...
if(isset($_POST['submit']))
{
  ...
  $name=filter_var($_POST['rfca'],FILTER_SANITIZE_STRING);
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST")
  {
    ...
    $myusername = mysqli_real_escape_string($conn,$_POST['rfca']);
    $mypassword = mysqli_real_escape_string($conn,$_POST['Contraseña']); 
    $mypassword=hash('sha512', $mypassword);
      
    $sql = "select nombreAd from administradores where idAdmin = '$myusername' and contrasenaAd = '$mypassword'";
    ...
    if($count == 1) 
    {
      ...
      mysqli_close($conn);
      header("location: admins_principal.php");
    }
    else
    {
      $errors[]="RFC/Contraseña no válidos <br>";
...
?>
	...
	<div class="col-xs-12 col-sm-8 col-md-8">
		<h1><br>Accede como administrador</h1>
 ...
 <div class="form-group">
    <label for="rfca" class="control-label">RFC:</label>
    <div>
    <input type="text" class="form-control required"  maxlength="14" name="rfca" placeholder="Teclee su rfc">
</div>
    <small id="rfcaHelp" class="form-text text-muted"></small>
  </div><!-- RFC-->
